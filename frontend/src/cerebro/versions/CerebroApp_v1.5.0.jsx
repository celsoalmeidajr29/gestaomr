// v1.5.0 — Email: responder a todos + anexos; WhatsApp: duas abas configuráveis
...
import {
  ...
  Mail, Send, Archive, Eye, BookOpen, PenLine, PlusCircle, Moon, Sun,
  Paperclip, MessageSquare, ReplyAll,
} from 'lucide-react'
...
function AbaNotionNotas() {
  ...
  const [newParentId, setNewParentId] = useState(() => localStorage.getItem('cerebro_notion_parent_id') || '')
  ...
  async function criarPagina() {
    ...
    if (!newParentId.trim()) { alert('Informe o ID da página pai no Notion (cole o ID da URL da página onde deseja criar).'); return }
    localStorage.setItem('cerebro_notion_parent_id', newParentId.trim())
    setLoading(true)
    try {
      await apiFetch('/notion.php?action=create', { method: 'POST', body: JSON.stringify({ title: newTitle, parent_id: newParentId.trim() }) })
    ...
    <div>
      ...
      {creating && (
        <div className="mb-4 flex flex-col gap-2">
          ...
          <div className="flex gap-2 items-center">
            <input value={newParentId} onChange={e => setNewParentId(e.target.value)}
              placeholder="ID da página pai no Notion (da URL — obrigatório)..."
              className="flex-1 rounded-xl px-4 py-2 text-sm outline-none"
              style={{ background: C.card, border: `1px solid ${C.border}`, color: C.text }} />
            <Btn variant="accent" disabled={!newTitle.trim() || !newParentId.trim() || loading} onClick={criarPagina}>Criar</Btn>
...
function ModalComporEmail({ initialTo = '', initialCc = '', initialSubject = '', threadId, replyAll = false, onSend, onClose }) {
  ...
  async function handleAddFile(e) {
    ...
    reader.onload = ev => {
      ...
      setFiles(prev => [...prev, { name: file.name, mimeType: file.type || 'application/octet-stream', data: base64 }])
    }
  ...
  async function handleSend() {
    ...
    setSending(true)
    try { await onSend(to, subject, body, threadId, cc, files) }
    catch (e) { alert('Erro ao enviar: ' + e.message); setSending(false) }
  }

  const title = threadId ? (replyAll ? 'Responder a todos' : 'Responder') : 'Novo e-mail'
  return (
    <ModalWrap title={title} onClose={onClose}>
      ...
      <Field label="CC"><FInput value={cc} onChange={e => setCc(e.target.value)} placeholder="Opcional" /></Field>
      <Field label="Assunto"><FInput value={subject} onChange={e => setSubject(e.target.value)} /></Field>
      <Field label="Mensagem">
        <FTA value={body} onChange={e => setBody(e.target.value)} rows={7} placeholder="Escreva sua mensagem..." />
      </Field>
      <div>
        ...
        {files.length > 0 && (
          <div className="mt-2 space-y-1">
            {files.map((f, i) => (
              <div key={i} className="flex items-center gap-2 text-xs px-2 py-1 rounded" style={{ background: C.surface, color: C.muted }}>
                <Paperclip className="w-3 h-3 flex-shrink-0" />
                <span className="flex-1 truncate">{f.name}</span>
                <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} style={{ color: C.muted }}>
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
...
function AbaEmail({ slots = {}, onConnectSlot, onDisconnectSlot }) {
  ...
  async function enviar(to, subject, body, threadId, cc = '', attachments = []) {
    await apiFetch(`/gmail.php?slot=${activeSlot}`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    return (
      <div>
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <Btn variant="default" onClick={() => setComposing({ replyTo: lendo, replyAll: false })}>
            <Send className="w-3.5 h-3.5" /> Responder
          </Btn>
          <Btn variant="default" onClick={() => setComposing({ replyTo: lendo, replyAll: true })}>
            <ReplyAll className="w-3.5 h-3.5" /> Resp. a todos
          </Btn>
          {caixa === 'inbox' && (
        ...
        <Card className="mb-4">
          ...
          <div className="space-y-1 text-xs">
            ...
            {lendo.cc && <p style={{ color: C.muted }}><span style={{ color: C.accent2 }}>CC:</span> {lendo.cc}</p>}
        ...
        {composing && (
          <ModalComporEmail
            ...
            initialCc={composing.replyAll ? [lendo.to, lendo.cc].filter(Boolean).join(', ') : ''}
...
// ---- WhatsApp ----
// ============================================================
const WA_LABELS_KEY = 'cerebro_whatsapp_labels'
function AbaWhatsApp() {
  const [labels, setLabels] = useState(() => {
    try { return JSON.parse(localStorage.getItem(WA_LABELS_KEY)) || ['WhatsApp 1', 'WhatsApp 2'] }
    catch { return ['WhatsApp 1', 'WhatsApp 2'] }
  })
  const [editIdx, setEditIdx] = useState(null)
  const [editVal, setEditVal] = useState('')

  function saveLabel(idx) {
    const next = [...labels]; next[idx] = editVal.trim() || labels[idx]
    setLabels(next); localStorage.setItem(WA_LABELS_KEY, JSON.stringify(next)); setEditIdx(null)
  }

  return (
    <div className="space-y-6">
      <p className="text-sm" style={{ color: C.muted }}>
        Clique para abrir o WhatsApp Web numa nova aba. WhatsApp Web não permite incorporação por restrições de segurança.
      </p>
        ...
        {[0, 1].map(i => (
          <Card key={i} className="p-5 flex flex-col gap-4">
              ...
              {editIdx === i ? (
                <>
                  <input
                    ...
                    onKeyDown={e => { if (e.key === 'Enter') saveLabel(i); if (e.key === 'Escape') setEditIdx(null) }}
                    className="flex-1 rounded-lg px-2 py-1 text-sm outline-none"
                    style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.text }}
                  />
                  <Btn variant="accent" onClick={() => saveLabel(i)}>
                    <Check className="w-3.5 h-3.5" />
                  </Btn>
                  <Btn variant="ghost" onClick={() => setEditIdx(null)}>
                    <X className="w-3.5 h-3.5" />
                  </Btn>
                </>
              ) : (
                <>
                  ...
                  <span className="font-semibold text-sm flex-1" style={{ color: C.text }}>{labels[i]}</span>
                  <button onClick={() => { setEditIdx(i); setEditVal(labels[i]) }}
                    className="p-1 rounded hover:opacity-70 transition" style={{ color: C.muted }}>
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 w-full rounded-xl py-3 font-semibold text-sm transition hover:opacity-90 active:scale-95"
            ...
            <p className="text-[11px] text-center" style={{ color: C.muted }}>
              Use contas diferentes em cada aba do navegador
            </p>
          </Card>
        ))}
      </div>
...
export default function CerebroApp({ usuario, onVoltarHub, onLogout }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: C.bg, color: C.text, fontFamily: "'Outfit', sans-serif" }}>
      <header className="sticky top-0 z-10 shadow-sm" style={{ borderBottom: `1px solid ${C.border}`, background: '#ffffff' }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={onVoltarHub} className="p-2 rounded-lg transition hover:bg-slate-100" style={{ color: C.muted }}><ArrowLeft className="w-5 h-5" /></button>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setDarkMode(v => !v)} title={darkMode ? 'Modo claro' : 'Modo escuro'} className="p-2 rounded-lg hover:bg-slate-100 transition" style={{ color: C.muted }}>
              {darkMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
            <button onClick={onLogout} className="text-sm px-3 py-1.5 rounded hover:bg-slate-100 transition flex items-center gap-1.5" style={{ color: C.muted }}>
