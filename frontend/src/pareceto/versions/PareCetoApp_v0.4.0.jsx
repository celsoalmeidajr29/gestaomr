// v0.4.0 — Top 30 Placas: critério irregulares não-pagas (era Top 20 por total)
...
// Busca paginada: acumula todas as páginas automaticamente.
// onProgress(carregados, total) chamado a cada página.
async function apiFetchPaginado(path, onProgress) {
  let all = []
  let page = 1
  let hasMore = true
  while (hasMore) {
    const sep = path.includes('?') ? '&' : '?'
    const r = await fetch(`${API}${path}${sep}page=${page}`, { credentials: 'include' })
    if (!r.ok) {
      const e = await r.json().catch(() => ({}))
      throw new Error(e.error || `HTTP ${r.status}`)
    }
    ...
    const total = parseInt(r.headers.get('X-Total-Count') || '0') || null
    all = all.concat(rows)
...
function analyzeIrregularidades(records) {
  ...
  const top30Placas = Object.values(porPlaca).map(p => ({
  ...
  })).sort((a, b) => b.irregular - a.irregular).slice(0, 30)
  ...
  return {
    ...
    top30Placas, top20Placas: top30Placas, porOrigem,
    dataMin: datas.length ? new Date(Math.min(...datas)) : null,
    dataMax: datas.length ? new Date(Math.max(...datas)) : null,
  }
}
...
function ComparacaoPeriodos({ records, tipo }) {
  ...
  function PeriodPicker({ label, color, p, setP }) {
    return (
      <div className={`bg-white border rounded-xl p-4 ${color === 'A' ? 'border-blue-500/30' : 'border-purple-500/30'}`}>
        <div className={`text-xs font-bold uppercase tracking-wider mb-3 ${color === 'A' ? 'text-blue-600' : 'text-purple-400'}`}>{label}</div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2 pb-2 border-b border-slate-200">
              ...
              <div className="flex gap-2">
                <span className="w-24 text-right text-blue-600">A</span>
...
function exportRelatorioSemanalTXT(semana, analise, top20, scorePlacas) {
  const L = [
    ...
    '', 'TOP 30 PLACAS INADIMPLENTES', '-'.repeat(55),
...
function RelatorioSemanal({ records, scorePlacas }) {
  ...
  const top20 = useMemo(() => {
    ...
    return Object.values(m).sort((a, b) => b.irregular - a.irregular).slice(0, 30)
  }, [recSemana])
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div>
          ...
          <select value={semana} onChange={e => setSemana(e.target.value)}
            className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500">
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="bg-white border border-slate-200/50 rounded-xl p-3">
                <div className="text-xs text-slate-400 mb-1">{label}</div>
                <div className="text-xl font-bold text-slate-900">{fmt ? fmt(val) : val.toLocaleString('pt-BR')}</div>
                {prev != null && (
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <div className="px-4 py-3 bg-white border-b border-slate-200 flex items-center justify-between">
              <span className="text-sm font-semibold text-slate-700">Top 30 placas inadimplentes — {semana}</span>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {top20.map((p, i) => {
                  const s = scorePlacas?.[p.placa]
                  return (
                    <tr key={p.placa} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5 text-right text-amber-600 font-medium">{p.irregular}</td>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-700/40">
                    {analise.rankingEmissores.slice(0, 10).map((e, i) => (
                      <tr key={e.nome} className="hover:bg-slate-100/20">
                        ...
                        <td className="py-2 text-right text-emerald-600">{e.paga}</td>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-800">
                  {analise.porTrecho.slice(0,10).map((t,i) => (
                    <tr key={t.trecho} className="hover:bg-slate-50">
                      <td className="px-4 py-2.5 text-slate-500 text-xs">{i+1}</td>
                      <td className="px-4 py-2.5 font-medium">{t.trecho}</td>
                      <td className="px-4 py-2.5 text-right text-amber-600">{t.count.toLocaleString('pt-BR')}</td>
          ...
          <div className="flex flex-wrap gap-2">
            <div className="relative flex-1 min-w-[160px]">
              ...
              <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar placa..."
                className="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-emerald-500"/>
            </div>
                ...
                {placasFiltradas.slice(0, 100).map((p, i) => {
                  ...
                  return (
                    <tr key={p.placa} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5 text-right text-amber-600 font-medium">{p.count}</td>
      ...
      <div className="flex flex-wrap gap-3 items-start">
        ...
        <div>
          ...
          <button onClick={() => setShowSelector(s=>!s)}
            className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm hover:bg-slate-100 transition">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {metricas.sort((a,b) => (b[metrica]??0)-(a[metrica]??0)).map((m,i) => (
                  <tr key={m.trecho} className="hover:bg-slate-50">
...
function DropZone({ onFile, label }) {
  ...
  return (
    <div
      className="border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center cursor-pointer hover:border-emerald-600/50 hover:bg-slate-50 transition"
...
function ModalFuncionario({ data, mode, onSalvar, onFechar }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 overflow-y-auto">
      <div className="bg-white border border-slate-300 text-slate-900 rounded-2xl w-full max-w-md p-6 shadow-2xl my-auto">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-semibold">{mode === 'criar' ? 'Novo funcionário' : 'Editar funcionário'}</h3>
          <button onClick={onFechar} className="p-1.5 hover:bg-slate-100 rounded text-slate-400 hover:text-slate-900"><X className="w-4 h-4" /></button>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">Nome completo *</label>
            <input value={form.nome} onChange={e => set('nome', e.target.value)}
              className="w-full px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg text-slate-900 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
          </div>
          <div>
            ...
            <input value={form.login} onChange={e => set('login', e.target.value)}
              className="w-full px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg font-mono text-slate-900 focus:outline-none focus:ring-1 focus:ring-emerald-500"
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onFechar} className="px-4 py-2 text-sm rounded-lg border border-slate-200 hover:bg-slate-100 text-slate-600 transition">Cancelar</button>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={onExportXLSX} className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-slate-200 hover:bg-slate-100 transition text-slate-600">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Nome ou login..."
            className="w-full pl-9 pr-3 py-2 text-sm bg-white border border-slate-200 rounded-lg text-slate-900 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
        </div>
        <select value={filtroCargo} onChange={e => setFiltroCargo(e.target.value)}
          className="px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg text-slate-600 focus:outline-none focus:ring-1 focus:ring-emerald-500">
              ...
              {funcionarios.map(f => (
                <tr key={f.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1.5 text-xs ${f.status === 'Ativo' ? 'text-emerald-600' : 'text-slate-500'}`}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => onEditar(f)} className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-slate-900 transition"><Edit2 className="w-4 h-4" /></button>
                      {f.status === 'Ativo' && (
                        <button onClick={() => onInativar(f.id)} className="p-1.5 rounded hover:bg-red-900/50 text-slate-400 hover:text-red-600 transition"><Trash2 className="w-4 h-4" /></button>
                      )}
                    </div>
                  </td>
                </tr>
    ...
    <div className="flex gap-1 mb-5 overflow-x-auto border-b border-slate-200">
      {tabs.map(([id, label]) => (
        <button key={id} onClick={() => setAba(id)}
          className={`px-4 py-2 text-sm font-medium border-b-2 transition whitespace-nowrap ${aba === id ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
          {label}
        </button>
      ))}
    </div>
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
      ...
      <div className="flex flex-wrap gap-2">
        {onNovoArquivo && (
          <>
            <button onClick={() => ref.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-slate-200 hover:bg-slate-100 text-slate-600 transition">
        ...
        {onExportTXT && (
          <button onClick={onExportTXT} className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-slate-200 hover:bg-slate-100 text-slate-600 transition">
            <FileText className="w-4 h-4" />TXT
          </button>
        )}
        {onExportXLSX && (
          <button onClick={onExportXLSX} className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-slate-200 hover:bg-slate-100 text-slate-600 transition">
            <Download className="w-4 h-4" />XLSX
          </button>
        )}
        {onSalvar && (
          ...
          <tbody className="divide-y divide-slate-800">
            {series.map(row => (
              <tr key={row.key} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-right text-emerald-600">{fmtBRL(row.valor)}</td>
...
function VendasFuncionarioDetalhe({ nome, cargo, records, analise, onVoltar }) {
  ...
  if (!a) return (
    <div className="space-y-4">
      <button onClick={onVoltar} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-emerald-600 transition-colors"><ArrowLeft className="w-4 h-4"/>Voltar</button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 pb-2 border-b border-slate-200/50">
        <button onClick={onVoltar} className="p-1.5 rounded-lg hover:bg-slate-100/60 text-slate-400 hover:text-emerald-600 transition-colors flex-shrink-0"><ArrowLeft className="w-4 h-4"/></button>
        ...
        <div className="ml-auto text-right flex-shrink-0">
          <div className="text-2xl font-bold text-emerald-600">#{a.ranking}</div>
...
function PeriodoDBSelector({ periodo, onRecarregar, loading, label = 'Período do banco', progresso = null }) {
  const [de,  setDe]  = useState(periodo?.de  || '')
  const [ate, setAte] = useState(periodo?.ate || '')
  function aplicar() { if (de && ate && de <= ate) onRecarregar({ de, ate }) }
  function atalho(dias) {
    const a = new Date(); const d = new Date(); d.setDate(d.getDate() - dias)
    const de2 = d.toISOString().slice(0,10); const ate2 = a.toISOString().slice(0,10)
    setDe(de2); setAte(ate2); onRecarregar({ de: de2, ate: ate2 })
  }
  function mesAtual() {
    const now = new Date()
    const de2 = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0,10)
    const ate2 = new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString().slice(0,10)
    setDe(de2); setAte(ate2); onRecarregar({ de: de2, ate: ate2 })
  }
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          <input type="date" value={de} onChange={e => setDe(e.target.value)}
            className="bg-white border border-slate-300 text-slate-900 rounded px-2 py-1 text-xs text-slate-700" />
          <span className="text-slate-600 text-xs">→</span>
          <input type="date" value={ate} onChange={e => setAte(e.target.value)}
            className="bg-white border border-slate-300 text-slate-900 rounded px-2 py-1 text-xs text-slate-700" />
          <button onClick={aplicar} disabled={loading || !de || !ate}
            className="px-2 py-1 rounded bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-xs flex items-center gap-1">
            {loading ? <RefreshCw className="w-3 h-3 animate-spin"/> : <RefreshCw className="w-3 h-3"/>} Carregar
          </button>
        </div>
        <div className="flex gap-1">
          <button onClick={mesAtual} disabled={loading}
      ...
      {loading && progresso && (
        <div className="flex flex-col gap-1">
          <div className="flex justify-between text-[11px] text-slate-400">
            <span><RefreshCw className="w-3 h-3 animate-spin inline mr-1"/>
              {progresso.carregados.toLocaleString('pt-BR')} registros carregados
              {progresso.total ? ` de ${progresso.total.toLocaleString('pt-BR')}` : '...'}
            </span>
...
function AbaVendas({ funcionarios, dados, premissas, analise, jornada, subAba, setSubAba, onUpload, loading, periodo, onRecarregarDB, progresso }) {
  ...
  if (!dados) {
    ...
    return (
      <div>
        ...
        <p className="text-sm text-slate-400 mb-4">Importe o CSV de vendas ou carregue do banco de dados.</p>
        {onRecarregarDB && periodo && (
  ...
  return (
    <div>
      ...
      />
      {onRecarregarDB && periodo && (
        <PeriodoDBSelector periodo={periodo} onRecarregar={onRecarregarDB} loading={loading} progresso={progresso} />
      )}
      {premissas.length > 0 && (
        <div className="mb-4 p-3 rounded-lg bg-white border border-slate-200 text-xs text-slate-400">
          <strong className="text-slate-600">Premissas:</strong> {premissas.join(' · ')}
        </div>
      )}
      <FilterBar
...
function RankingRow({ label, count, total, valor, alert }) {
  ...
  return (
    <div className={`py-1.5 border-b border-slate-200/40 last:border-0 ${alert ? 'text-red-600' : ''}`}>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-800">
          {a.rankingAgentes.map((ag, i) => (
            <tr key={ag.nome} className={`hover:bg-slate-50 ${ag.cargo === 'Não cadastrado' ? 'text-amber-600/80' : ''}`}>
              <td className="px-4 py-2.5 text-slate-500 text-xs">{i+1}</td>
              ...
              <td className="px-4 py-2.5 text-right text-emerald-600">{fmtBRL(ag.valor)}</td>
        ...
        <tbody className="divide-y divide-slate-800">
          {a.rankingTrechos.map((t, i) => (
            <tr key={t.trecho} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-2.5 text-right text-emerald-600">{fmtBRL(t.valor)}</td>
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-slate-200">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-800">
          {jornada.map((j, i) => (
            <tr key={i} className={`hover:bg-slate-50 ${j.pausasCriticas.length > 0 ? 'bg-red-900/10' : ''}`}>
              ...
              <td className={`px-4 py-2.5 text-right font-medium ${j.pausasCriticas.length > 0 ? 'text-red-600' : 'text-slate-500'}`}>{j.pausasCriticas.length || '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {porFaixa.map((f, i) => (
        <div key={f.faixa} className="bg-white border border-slate-200 rounded-xl p-3 text-center">
          ...
          <div className="text-base font-bold text-emerald-600">{f.count.toLocaleString('pt-BR')}</div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            {a.porFormaPag.map((fp, i) => (
              <tr key={fp.name} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-right text-emerald-600">{fmtBRL(fp.valor)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            {a.rankingAgentes.map((ag, i) => (
              <tr
                key={ag.nome}
                className={`transition-colors hover:bg-white ${onSelectAgente ? 'cursor-pointer' : ''} ${ag.cargo === 'Não cadastrado' ? 'text-amber-600/80' : ''}`}
...
// ---- ABA IRREGULARIDADES ----
function AbaIrregularidades({ funcionarios, dados, premissas, analise, subAba, setSubAba, onUpload, loading, scorePlacas, alertaDismissed, onDismissAlerta, scoreConfig, periodo, onRecarregarDB, progresso }) {
  const [filtroMes, setFiltroMes] = useState('')
  const [filtroDe, setFiltroDe] = useState('')
  const [filtroAte, setFiltroAte] = useState('')
  ...
  if (!dados) {
    ...
    return (
      <div>
        ...
        <p className="text-sm text-slate-400 mb-4">Importe o CSV de irregularidades ou carregue do banco de dados.</p>
        {onRecarregarDB && periodo && (
          <PeriodoDBSelector periodo={periodo} onRecarregar={onRecarregarDB} loading={loading} label="Carregar do banco" progresso={progresso} />
        )}
        {funcionarios.length === 0 && (
  ...
  return (
    <div>
      ...
      />
      {onRecarregarDB && periodo && (
        <PeriodoDBSelector periodo={periodo} onRecarregar={onRecarregarDB} loading={loading} label="Período do banco" progresso={progresso} />
      )}
      {premissas.length > 0 && (
        <div className="mb-4 p-3 rounded-lg bg-white border border-slate-200 text-xs text-slate-400">
          <strong className="text-slate-600">Premissas:</strong> {premissas.join(' · ')}
      ...
      />
      <SubTabs tabs={[['kpis','KPIs'],['risco','Score / Risco'],['inadimplencia','Inadimplência'],['semanas','Por Semana'],['emissores','Emissores'],['trechos','Trechos'],['placas','Top 30 Placas'],['comparar','Comparar'],['trechos-comp','Comp. Trechos'],['relatorio','Rel. Semanal']]} aba={subAba} setAba={setSubAba} />
        ...
        <tbody className="divide-y divide-slate-800">
          {a.porSemana.map(s => (
            <tr key={s.label} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-2.5 text-right text-amber-600">{s.irregular}</td>
              <td className="px-4 py-2.5 text-right text-emerald-600">{s.paga}</td>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-800">
          {a.rankingEmissores.map((e, i) => (
            <tr key={e.nome} className={`hover:bg-slate-50 ${e.cargo === 'Não cadastrado' ? 'text-amber-600/80' : ''}`}>
              <td className="px-4 py-2.5 text-slate-500 text-xs">{i+1}</td>
              ...
              <td className="px-4 py-2.5 text-right text-emerald-600">{e.paga}</td>
        ...
        <tbody className="divide-y divide-slate-800">
          {a.rankingTrechos.map((t, i) => (
            <tr key={t.trecho} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-2.5 text-right text-emerald-600">{t.paga}</td>
          ...
          {a.top20Placas.map((p, i) => {
            ...
            return (
              <tr key={p.placa} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-right text-amber-600">{p.irregular}</td>
                <td className="px-4 py-2.5 text-right text-emerald-600">{p.paga}</td>
                <td className="px-4 py-2.5 text-right">{fmtBRL(p.valor)}</td>
            ...
            {sorted.map((p, i) => (
              <tr key={p.placa} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-right">
                  {p.reincidente
                    ? <span className="inline-flex items-center gap-1 text-xs text-red-600"><Bell className="w-3 h-3"/>{p.rec90}x</span>
  ...
  return (
    <div className="mb-4 rounded-xl border border-red-500/30 bg-red-500/8 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-red-500/10 border-b border-red-500/20">
        ...
        <button onClick={onDismiss} className="text-slate-500 hover:text-slate-600 p-1">
          <X className="w-4 h-4" />
        </button>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={onRecarregar} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-900 transition" title="Recarregar">
            ...
            <tbody className="divide-y divide-slate-800">
              {historico.map(r => (
                <tr key={r.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => onExcluir(r.id)} className="p-1.5 rounded hover:bg-red-900/50 text-slate-500 hover:text-red-600 transition">
            ...
            <table className="w-full text-sm">
              <tbody className="divide-y divide-slate-800">
                {analiseVendas.rankingAgentes.slice(0,5).map((ag,i) => (
                  <tr key={ag.nome} className="hover:bg-slate-50">
                    <td className="px-4 py-2.5 text-slate-500 text-xs">{i+1}</td>
                    <td className="px-4 py-2.5 font-medium truncate">{ag.nome}</td>
                    <td className="px-4 py-2.5 text-right text-emerald-600 whitespace-nowrap">{ag.count.toLocaleString('pt-BR')} / {fmtBRL(ag.valor)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody className="divide-y divide-slate-800">
                {analiseIrreg.rankingTrechos.slice(0,5).map((t,i) => (
                  <tr key={t.trecho} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2.5 text-right text-blue-600 whitespace-nowrap">{t.total.toLocaleString('pt-BR')} / {fmtBRL(t.valor)}</td>
                    <td className="px-4 py-2.5 text-right"><ConvBar pct={t.total>0?t.paga/t.total*100:0}/></td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-6 mt-4">
              ...
              {PESOS.map(({ key, label, desc }) => (
                <div key={key}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-bold text-emerald-600 w-8 text-right">{cfg[key]}</span>
                  </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { setCfg({ ...DEFAULT_SCORE_CONFIG }); setDirty(true) }}
              className="ml-auto text-xs text-slate-500 hover:text-slate-600 transition">
              Restaurar padrão
            </button>
          </div>
        </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                ...
                {metas.map(m => (
                  <tr key={m.id || `${m.funcionario_nome}${m.mes}`} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => { setEditandoMeta(m); setMetaForm({ funcionario_nome: m.funcionario_nome, mes: m.mes, meta_trans: m.meta_trans || '', meta_valor: m.meta_valor || '' }) }}
                          className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-slate-900 transition"><Edit2 className="w-3.5 h-3.5"/></button>
                        <button onClick={() => onDeleteMeta(m.id)} className="p-1.5 rounded hover:bg-red-900/40 text-slate-400 hover:text-red-600 transition"><Trash2 className="w-3.5 h-3.5"/></button>
                      </div>
                    </td>
                  </tr>
                ))}
...
export default function PareCetoApp({ usuario, onVoltarHub, onLogout }) {
  ...
  const [progressoVendas, setProgressoVendas] = useState(null) // { carregados, total } | null
  ...
  const [progressoIrreg, setProgressoIrreg] = useState(null)  // { carregados, total } | null
  ...
  const [periodoVendas, setPeriodoVendas] = useState(defaultPeriodo)
  const [periodoIrreg,  setPeriodoIrreg]  = useState(defaultPeriodo)

  async function carregarVendas(periodo) {
    const p = periodo || periodoVendas
    setLoadingVendas(true)
    setProgressoVendas({ carregados: 0, total: null })
    try {
      const qs = `?de=${p.de}&ate=${p.ate}`
      const rows = await apiFetchPaginado('/vendas/index.php' + qs, (carregados, total) => {
        setProgressoVendas({ carregados, total })
      })
      const records = dbRowsToVendas(rows)
      setDadosVendas({ records, nomeArquivo: `banco · ${p.de} → ${p.ate}` })
      if (periodo) setPeriodoVendas(periodo)
    } catch (e) { showToast('Erro ao carregar vendas: ' + e.message, 'erro') }
    finally { setLoadingVendas(false); setProgressoVendas(null) }
  }
  async function carregarIrreg(periodo) {
    const p = periodo || periodoIrreg
    setLoadingIrreg(true)
    setProgressoIrreg({ carregados: 0, total: null })
    try {
      const qs = `?de=${p.de}&ate=${p.ate}`
      const rows = await apiFetchPaginado('/irregularidades/index.php' + qs, (carregados, total) => {
        setProgressoIrreg({ carregados, total })
      })
      const records = dbRowsToIrreg(rows)
      setDadosIrreg({ records, nomeArquivo: `banco · ${p.de} → ${p.ate}` })
      if (periodo) setPeriodoIrreg(periodo)
    } catch (e) { showToast('Erro ao carregar irregularidades: ' + e.message, 'erro') }
    finally { setLoadingIrreg(false); setProgressoIrreg(null) }
  }
  const dashboardAutoCarregado = useRef(false)
  useEffect(() => {
    if (aba === 'dashboard' && !dashboardAutoCarregado.current) {
      dashboardAutoCarregado.current = true
      carregarVendas(); carregarIrreg()
    }
  ...
  async function autoRegistrarFuncionarios(nomes, cargo = 'Agente') {
    ...
    for (const nome of novos) {
      try {
        ...
        criados++
      } catch (e) { console.warn('autoRegistrar falhou:', nome, e?.message) }
    }
    if (criados > 0) showToast(`${criados} funcionário${criados > 1 ? 's' : ''} cadastrado${criados > 1 ? 's' : ''} automaticamente`, 'sucesso')
    return criados
  }
      ...
      <header className="border-b border-slate-200 bg-white sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={onVoltarHub} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-900 transition" title="Voltar ao Hub">
        ...
        <div className="max-w-7xl mx-auto px-4 flex gap-0 overflow-x-auto">
          {TABS.map(({ id, label, Icon, badge }) => (
            <button key={id} onClick={() => setAba(id)}
              className={`relative flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition whitespace-nowrap min-h-[40px] ${aba === id ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {aba === 'vendas' && (
          <AbaVendas
            ...
            periodo={periodoVendas} onRecarregarDB={carregarVendas} progresso={progressoVendas}
          />
        )}
        {aba === 'irregularidades' && (
          <AbaIrregularidades
            ...
            periodo={periodoIrreg} onRecarregarDB={carregarIrreg} progresso={progressoIrreg}
          />
        )}
        {aba === 'historico' && (
