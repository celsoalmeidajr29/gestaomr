// v0.1.1 — fix: carregamento de vendas/irreg com filtro de período padrão (evita HTTP 500 por excesso de dados)
...
export default function PareCetoApp({ usuario, onVoltarHub, onLogout }) {
  ...
  // Período padrão de carga: últimos 12 meses (evita 500 por excesso de dados no Hostinger)
  function defaultPeriodo() {
    const ate = new Date(); const de = new Date(); de.setFullYear(de.getFullYear() - 1)
  ...
  async function carregarVendas(periodo) {
    ...
    try {
      ...
      if (records.length) {
        setDadosVendas({ records, nomeArquivo: `${records.length.toLocaleString('pt-BR')} transações do banco (${p.de} → ${p.ate})` })
      } else if (!periodo) {
        // Sem dados no período padrão — tenta carregar tudo (banco possivelmente vazio ou dados antigos)
        const rowsAll = await apiFetch('/vendas/index.php')
        const recAll = dbRowsToVendas(rowsAll)
        if (recAll.length) setDadosVendas({ records: recAll, nomeArquivo: `${recAll.length.toLocaleString('pt-BR')} transações do banco` })
      }
  ...
  async function carregarIrreg(periodo) {
    ...
    try {
      ...
      if (records.length) {
        setDadosIrreg({ records, nomeArquivo: `${records.length.toLocaleString('pt-BR')} registros do banco (${p.de} → ${p.ate})` })
      } else if (!periodo) {
        const rowsAll = await apiFetch('/irregularidades/index.php')
        const recAll = dbRowsToIrreg(rowsAll)
        if (recAll.length) setDadosIrreg({ records: recAll, nomeArquivo: `${recAll.length.toLocaleString('pt-BR')} registros do banco` })
      }
