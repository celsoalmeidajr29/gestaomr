// v0.1.2 — fix: padrão 30 dias (era 12 meses) + seletor de período visível + aviso de truncamento (LIMIT 30k)
...
function PeriodoDBSelector({ periodo, onRecarregar, loading, label = 'Período do banco' }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4 p-3 rounded-xl bg-slate-800/60 border border-slate-700">
...
function AbaVendas({ funcionarios, dados, premissas, analise, jornada, subAba, setSubAba, onUpload, loading, periodo, onRecarregarDB, dbLimit }) {
    ...
    return (
      <div>
        ...
        {onRecarregarDB && periodo && (
          <PeriodoDBSelector periodo={periodo} onRecarregar={onRecarregarDB} loading={loading} label="Carregar do banco" />
        )}
        {funcionarios.length === 0 && (
  ...
  return (
    <div>
      ...
      />
      {onRecarregarDB && periodo && (
        <PeriodoDBSelector periodo={periodo} onRecarregar={onRecarregarDB} loading={loading} />
      )}
      {dbLimit && records.length >= dbLimit && (
        <div className="mb-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-center gap-2">
          ...
          Exibindo os {dbLimit.toLocaleString('pt-BR')} registros mais recentes — limite do servidor atingido. Reduza o período para ver todos os dados.
        </div>
      )}
      {premissas.length > 0 && (
...
// ---- ABA IRREGULARIDADES ----
function AbaIrregularidades({ funcionarios, dados, premissas, analise, subAba, setSubAba, onUpload, loading, scorePlacas, alertaDismissed, onDismissAlerta, scoreConfig, periodo, onRecarregarDB, dbLimit }) {
  const [filtroMes, setFiltroMes] = useState('')
  const [filtroDe, setFiltroDe] = useState('')
  const [filtroAte, setFiltroAte] = useState('')
  ...
  return (
    <div>
      ...
      />
      {onRecarregarDB && periodo && (
        <PeriodoDBSelector periodo={periodo} onRecarregar={onRecarregarDB} loading={loading} label="Período do banco" />
      )}
      {dbLimit && records.length >= dbLimit && (
        <div className="mb-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-center gap-2">
          <AlertTriangle className="w-4 h-4 flex-shrink-0" />
...
export default function PareCetoApp({ usuario, onVoltarHub, onLogout }) {
  ...
  const DB_LIMIT = 30000 // limite seguro para Hostinger (evita estouro de memória PHP)
  ...
  async function carregarVendas(periodo) {
    ...
    try {
      ...
      const rows = await apiFetch('/vendas/index.php' + qs)
  ...
  async function carregarIrreg(periodo) {
    ...
    try {
      const qs = `?de=${p.de}&ate=${p.ate}`
      const rows = await apiFetch('/irregularidades/index.php' + qs)
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {aba === 'vendas' && (
          <AbaVendas
            ...
            periodo={periodoVendas} onRecarregarDB={carregarVendas} dbLimit={DB_LIMIT}
          />
        )}
        {aba === 'irregularidades' && (
          <AbaIrregularidades
            ...
            periodo={periodoIrreg} onRecarregarDB={carregarIrreg} dbLimit={DB_LIMIT}
          />
        )}
        {aba === 'historico' && (
