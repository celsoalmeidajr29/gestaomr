// v0.1.5 — auto-cadastro de funcionários ao importar CSV (com toast de feedback)
...
function PeriodoDBSelector({ periodo, onRecarregar, loading, label = 'Período do banco', progresso = null }) {
  ...
  return (
    <div className="flex flex-col gap-2 mb-4 p-3 rounded-xl bg-slate-800/60 border border-slate-700">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex gap-1">
          <button onClick={mesAtual} disabled={loading}
            className="px-2 py-1 rounded bg-indigo-700 hover:bg-indigo-600 disabled:opacity-40 text-xs text-slate-200">mês atual</button>
          {[[7,'7d'],[30,'30d'],[60,'60d'],[90,'90d']].map(([d,l]) => (
            <button key={d} onClick={() => atalho(d)} disabled={loading}
              className="px-2 py-1 rounded bg-slate-700 hover:bg-slate-600 disabled:opacity-40 text-xs text-slate-300">{l}</button>
          ))}
        </div>
      </div>
      {loading && progresso && (
      ...
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-slate-500">
        ...
        {progresso && progresso.total ? (
          <div className="flex flex-col items-center gap-2 w-64">
            <span className="text-sm text-slate-300">{progresso.carregados.toLocaleString('pt-BR')} / {progresso.total.toLocaleString('pt-BR')} registros</span>
