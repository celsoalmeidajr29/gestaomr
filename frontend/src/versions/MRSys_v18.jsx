// v18 — lançamentos com breakdown detalhado + filtro de mês
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="max-w-7xl mx-auto px-2 sm:px-4 py-3 sm:py-6 print:p-0 print:max-w-none">
        ...
        {aba === 'lancamentos' && (() => {
          ...
          // Subtotais dos filtrados
          const tot = {
            bFat:    lancFiltrados.reduce((s, l) => s + bFat(l), 0),
            hxFat:   lancFiltrados.reduce((s, l) => s + num(l.extraHorasFatura), 0),
            kmFat:   lancFiltrados.reduce((s, l) => s + num(l.extraKmFatura), 0),
            outFat:  lancFiltrados.reduce((s, l) => s + num(l.adicDomFatura) + num(l.outros) + num(l.pedagioFatura) + num(l.batidaExtra), 0),
            fat:     lancFiltrados.reduce((s, l) => s + num(l.totalFatura), 0),
            bPago:   lancFiltrados.reduce((s, l) => s + bPago(l), 0),
            hxPago:  lancFiltrados.reduce((s, l) => s + num(l.extraHorasPaga), 0),
            kmPago:  lancFiltrados.reduce((s, l) => s + num(l.extraKmPago), 0),
            outPago: lancFiltrados.reduce((s, l) => s + num(l.adicDomPago) + num(l.pedagioReembolso), 0),
            pag:     lancFiltrados.reduce((s, l) => s + num(l.totalPago), 0),
            luc:     lancFiltrados.reduce((s, l) => s + num(l.lucro), 0),
          };
                  ...
                  <table className="w-full text-sm">
                    <thead className="text-xs bg-slate-900 border-b border-slate-800">
                      ...
                      <tr className="text-slate-500 text-[10px] uppercase font-semibold border-b border-slate-800/60">
                        <th rowSpan={2} className="text-left py-2 px-3 text-slate-400">Data</th>
                    ...
                    <tbody>{lancFiltrados.map(l => {
                      ...
                      return (
                      <tr key={l.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 text-xs">
                        <td className="py-2 px-3 whitespace-nowrap">{fmtData(l.data)}</td>
                        ...
                        {temHxFat  && <td className={`text-right px-2 ${hxF > 0 ? 'text-emerald-300' : 'text-slate-600'}`}>{hxF > 0 ? fmt(hxF) : '—'}</td>}
                        {temKmFat  && <td className={`text-right px-2 ${kmF > 0 ? 'text-emerald-300' : 'text-slate-600'}`}>{kmF > 0 ? fmt(kmF) : '—'}</td>}
                    ...
                    <tfoot className="bg-slate-900 border-t-2 border-slate-700 font-bold text-xs">
                      <tr>
                        <td colSpan={2} className="py-2.5 px-3 text-slate-400">TOTAL ({lancFiltrados.length})</td>
