// v17 — modais mais horizontais, checkboxes+ações em massa, cópia de serviço
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>{servicosFiltrados.map(s => { const cat = s.categoriaServico || 'VELADA'; const corC = CORES_CATEGORIA_SERVICO[cat] || CORES_CATEGORIA_SERVICO.VELADA; const sel = selServicos.has(s.cod); return (
    ...
    <ModalBase titulo={isEdit ? 'Editar serviço' : 'Novo serviço'} onClose={onClose} grande>
      ...
      <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-lg p-3 mb-3 flex flex-col sm:flex-row gap-3 items-start sm:items-end">
        ...
        <div className="flex gap-3 sm:gap-4">
          <Campo label="Categoria"><select value={f.categoriaServico || 'VELADA'} onChange={e => setF({ ...f, categoriaServico: e.target.value })} className="bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm">{CATEGORIAS_SERVICO.map(c => <option key={c} value={c}>{c}</option>)}</select></Campo>
