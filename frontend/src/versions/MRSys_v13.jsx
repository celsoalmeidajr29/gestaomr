...
import {
  ...
  Database, Save, X, Search, TrendingUp, DollarSign,
  ...
  Camera, Paperclip, FileCheck, User
} from 'lucide-react';
...
const CATEGORIAS_SERVICO = ['ARMADA', 'VELADA', 'MOTOLINK', 'PRONTA RESPOSTA', 'FACILITIES'];
const CORES_CATEGORIA_SERVICO = {
  'ARMADA': { bg: 'bg-red-500/20', text: 'text-red-300', border: 'border-red-500/40' },
  'VELADA': { bg: 'bg-slate-500/20', text: 'text-slate-300', border: 'border-slate-500/40' },
...
function diffHorasDecimal(ini, fim) {
  if (!ini || !fim) return 0;
...
function calcular(s, l, t) {
  ...
  const horasExtras = Math.max(0, ht - num(s.franquiaHoras));
  const kmExtras = Math.max(0, km - num(s.franquiaKm));
  const isDom = l.isDomingo === undefined ? eDomingo(l.data) : !!l.isDomingo;
  const extraHorasFatura = horasExtras * num(s.horaExtraFatura);
  const extraKmFatura = kmExtras * num(s.kmExtraFatura);
  const adicDomFatura = isDom ? num(s.adicionalDomingosFatura) : 0;
  const extraHorasPaga = horasExtras * num(s.horaExtraPaga);
  const extraKmPago = kmExtras * num(s.kmExtraPago);
  const adicDomPago = isDom ? num(s.adicionalDomingosPago) : 0;
  ...
  const totalFatura = num(s.valorFatura) + extraHorasFatura + extraKmFatura + adicDomFatura + outros + pedagioFatura + batidaExtra;
  const totalPago = num(s.diariaPaga) + extraHorasPaga + extraKmPago + adicDomPago + pedagioReembolso;
  // Alíquota (snapshot no momento do cálculo). Aplicada sobre o totalFatura, descontada do lucro.
  const aliquota = num(s.aliquota);
  const imposto = totalFatura * (aliquota / 100);
  return { horasExtras, kmExtras, isDom, extraHorasFatura, extraKmFatura, adicDomFatura, extraHorasPaga, extraKmPago, adicDomPago, outros, batidaExtra, pedagio: ped, pedagioFatura, pedagioReembolso, totalFatura, totalPago, aliquota, imposto, lucro: totalFatura - totalPago - imposto };
}
...
function valorParticipacao(funcionario, lanc) {
  const qtd = nomesNoLancamento(lanc).length || 1;
  if (num(funcionario.valorDiaria) > 0) return num(funcionario.valorDiaria);
  return num(lanc.totalPago) / qtd;
}
...
function exportarFaturaXLSX(fatura, servicos) {
  ...
  if (t?.exportFormat === 'natura') return exportarFaturaNaturaXLSX(fatura, servicos);
...
function exportarFaturaNaturaXLSX(fatura, servicos) {
  ...
  const headers = ['DATA', 'CLIENTE', 'CNPJ', 'COD. EMISSÃO', 'DIA', 'FERIADO', 'DOM/FER', 'MÊS', 'FRANQUIA', 'franq km', 'HORAS EXTRAS', 'BASE', 'TIPO', 'VLR BASE', 'VLR HORA EXTRA', 'KM. EXTRA', 'ADICIONAL', 'VALOR'];
  ...
  const rowsDet = itens.map(l => {
    ...
    return [
      ...
      l.isDomingo ? 1 : 0,
      MESES[Number(mes) - 1] || '',
      num(s?.franquiaHoras) > 0 ? fmtHorasHHMM(s.franquiaHoras) : '',
      num(s?.franquiaKm) || 0,
      num(l.horasExtras) > 0 ? Number(l.horasExtras.toFixed(2)) : '',
      e.base || '',
      e.tipoOp || '',
      num(s?.valorFatura) || 0,
      num(l.extraHorasFatura) || '',
      num(l.extraKmFatura) || '',
      num(l.adicDomFatura) || 0,
      num(l.totalFatura),
    ];
  });
  const sumCol = (key) => itens.reduce((s, l) => s + num(l[key]), 0);
  const valorBaseTotal = itens.reduce((s, l) => { const sv = servicos.find(x => x.cod === l.codServico); return s + num(sv?.valorFatura); }, 0);
  const totalRow = ['TOTAL', '', '', '', '', '', '', '', '', '', '', '', '', valorBaseTotal, sumCol('extraHorasFatura'), sumCol('extraKmFatura'), sumCol('adicDomFatura'), fatura.totalFatura];
  const ws1 = XLSX.utils.aoa_to_sheet([headers, ...rowsDet, totalRow]);
  ws1['!cols'] = [{ wch: 11 }, { wch: 26 }, { wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 8 }, { wch: 8 }, { wch: 11 }, { wch: 9 }, { wch: 9 }, { wch: 11 }, { wch: 9 }, { wch: 11 }, { wch: 11 }, { wch: 14 }, { wch: 11 }, { wch: 11 }, { wch: 12 }];
  ws1['!freeze'] = { ySplit: 1 };
  // Format money columns: VLR BASE(13), VLR HORA EXTRA(14), KM. EXTRA(15), ADICIONAL(16), VALOR(17)
  ...
  itens.forEach(l => {
    const e = l.extras || {};
    const k = `${e.base || '—'} | ${e.tipoOp || '—'}`;
    if (!resumoBase[k]) resumoBase[k] = { qtd: 0, fatura: 0 };
    resumoBase[k].qtd++; resumoBase[k].fatura += num(l.totalFatura);
  });
  const ws2Data = [
    ...
    ['CNPJ:', cnpj],
    ['Período:', fmtPeriodo(fatura.periodo, t)],
    ['Template:', t?.nome || ''],
    ...
    [],
    ['TOTAL FATURADO:', '', fatura.totalFatura],
  ];
  ...
  const cliSafe = `Natura_${t?.nome.replace(/[^a-zA-Z0-9]/g, '_')}`;
...
function exportarFaturaPadraoXLSX(fatura, servicos) {
  ...
  const sum = (key) => itens.reduce((s, l) => s + num(l[key]), 0);
  const valorBaseTotal = itens.reduce((s, l) => { const sv = servicos.find(x => x.cod === l.codServico); return s + num(sv?.valorFatura); }, 0);
  ...
  const totalRow = colsFiltered.map((c, i) => i === 0 ? 'TOTAL' : totalDeColuna(c.l));
  const ws1 = XLSX.utils.aoa_to_sheet([headers, ...rowsDet, totalRow]);
  ...
  const resumoServico = {};
  itens.forEach(l => { if (!resumoServico[l.codServico]) resumoServico[l.codServico] = { descricao: l.descricao, qtd: 0, fatura: 0 }; resumoServico[l.codServico].qtd++; resumoServico[l.codServico].fatura += num(l.totalFatura); });
  const composicao = [];
  ...
  const operacionais = [];
  if (sum('horasTrabalhadas') > 0) operacionais.push(['Total de horas trabalhadas:', fmtHorasHHMM(sum('horasTrabalhadas'))]);
  if (sum('horasExtras') > 0) operacionais.push(['Total de horas extras:', fmtHorasHHMM(sum('horasExtras'))]);
  if (sum('kmRodados') > 0) operacionais.push(['Total de KM rodados:', sum('kmRodados')]);
  if (sum('kmExtras') > 0) operacionais.push(['Total de KM extras:', sum('kmExtras')]);
  ...
  if (composicao.length) { ws2Data.push([], ['COMPOSIÇÃO DO FATURAMENTO'], ...composicao, ['TOTAL FATURADO:', fatura.totalFatura]); }
...
function exportarResumoFechamentoXLSX(resumo, competencia) {
  ...
  // ABA 1: FATURAMENTO POR CLIENTE
  const fatRows = [['LANÇAMENTO', '', 'VALOR', '', 'COD. EMISSÃO']];
  resumo.faturamento.forEach(f => {
    fatRows.push([f.lancamento, '', f.valor, '', f.emissao]);
  });
  fatRows.push([]);
  fatRows.push(['TOTAL', '', resumo.totalFaturamento]);
  const ws1 = XLSX.utils.aoa_to_sheet(fatRows);
  ws1['!cols'] = [{ wch: 36 }, { wch: 4 }, { wch: 18 }, { wch: 4 }, { wch: 16 }];
  ...
  // ABA 2: FOLHA POR FUNCIONÁRIO
  const folhaRows = [[mesNome, '', 'Origem']];
  resumo.folha.forEach(f => folhaRows.push([f.valor, '', f.origem || 'EMPRESA']));
  folhaRows.push([]);
  folhaRows.push([resumo.totalFolha, '', 'TOTAL']);
...
export default function App() {
  ...
  const salvarChave = useCallback(async (key, value, tentativa = 1) => {
    try {
      ...
      if (isQuota) {
        ...
        setErroSalvar({ key, mensagem: `Os dados de "${key}" estão grandes demais (${tamMB} MB, ${qtd} registros) para o storage do navegador. Use Backup → Exportar JSON e remova registros antigos, ou divida em arquivos separados.`, tipo: 'quota' });
        setSalvando(false);
  ...
  const agendarSalvamento = useCallback((key, value) => {
    // Pré-flight: se valor é exatamente igual ao último salvo, ignora.
    const sig = JSON.stringify(value).length + ':' + (Array.isArray(value) ? value.length : 0);
    if (ultimoSalvoRef.current[key] === sig) return;
  ...
  const lancFiltrados = useMemo(() => lancamentos.filter(l => {
    ...
    return true;
  }).sort((a, b) => b.data.localeCompare(a.data)), [lancamentos, filtroCliente, filtroStatus, busca]);
  ...
  const clientesUnicos = useMemo(() => [...new Set(servicos.map(s => s.cliente))].sort(), [servicos]);
  ...
  const faturas = useMemo(() => {
    ...
    lancamentos.forEach(l => {
      ...
      const k = `${l.cliente}|${l.template}|${periodo}`;
      if (!m[k]) m[k] = { cliente: l.cliente, template: l.template, periodo, lancs: [] };
      m[k].lancs.push(l);
    });
    return Object.values(m).map(f => {
      ...
      return { ...f, totalFatura: fat, totalPago: pag, lucro: fat - pag, qtd: f.lancs.length };
  ...
  const folhasPorFunc = useMemo(() => {
    ...
    return Object.values(m).map(g => {
      ...
      const totalVales = valesPeriodo.reduce((s, v) => s + num(v.valor), 0);
      ...
      const bruto = g.total + salarioFixoMes + adicionais;
      return { ...g, ajustes, vales: valesPeriodo, totalVales, adicionais, descontos: descontosTotal, descontosManuais, salarioFixo: salarioFixoMes, bruto, liquido: bruto - descontosTotal, status: folha?.status || 'aberta', folhaId: folha?.id };
    }).sort((a, b) => b.periodo.localeCompare(a.periodo) || a.funcionario.nome.localeCompare(b.funcionario.nome));
  }, [funcionarios, lancamentos, fechamentos, folhas, descontos]);
  ...
  const resumoFechamento = useMemo(() => {
    ...
    if (!cp) return null;
    // Faturamento: SOMENTE faturas geradas (fechamentos) com periodo === cp.
    // Expande os lançamentos contidos nos fechamentos da competência e agrupa por serviço.
    const fechCp = fechamentos.filter(f => f.periodo === cp);
    const idsFaturados = new Set();
    fechCp.forEach(f => (f.lancamentos || []).forEach(lid => idsFaturados.add(lid)));
    const lancsCp = lancamentos.filter(l => idsFaturados.has(l.id));
    const fatGroup = {};
    lancsCp.forEach(l => {
      const s = servicos.find(x => x.cod === l.codServico);
      const emissao = s?.emissao || '';
      const k = `${l.cliente}|${l.descricao}|${emissao}`;
      if (!fatGroup[k]) fatGroup[k] = { key: k, lancamento: `OP. ${l.descricao}`, cliente: l.cliente, valor: 0, imposto: 0, emissao, qtd: 0, codServico: l.codServico };
      fatGroup[k].valor += num(l.totalFatura);
      fatGroup[k].imposto += num(l.imposto);
      fatGroup[k].qtd += 1;
    });
    const faturamento = Object.values(fatGroup).sort((a, b) => b.valor - a.valor);
    const totalFaturamento = faturamento.reduce((s, f) => s + f.valor, 0);
    const totalImpostoFat = faturamento.reduce((s, f) => s + f.imposto, 0);
    const totalPagoFat = fechCp.reduce((s, f) => s + num(f.totalPago), 0);
    const numerosFaturas = fechCp.map(f => f.numeroFmt).filter(Boolean);

    // Folha por funcionário da competência cp (já filtrada por lançamentos faturados em folhasPorFunc)
    const folhaItems = folhasPorFunc.filter(f => f.periodo === cp).map(f => ({
      key: f.funcionario.id,
      funcionario: f.funcionario.nome,
      categoria: f.funcionario.categoria,
      valor: f.liquido,
      origem: 'EMPRESA',
      bruto: f.total,
      adicionais: f.adicionais,
      descontos: f.descontos,
    }));
    const totalFolha = folhaItems.reduce((s, f) => s + num(f.valor), 0);
    ...
    const totalAdiantamentos = adiantamentos.reduce((s, d) => s + num(d.valor), 0);
    ...
    const totalFixas = despesasFixas.reduce((s, d) => s + num(d.valor), 0);
    const totalParcelamentos = parcelamentos.reduce((s, d) => s + num(d.valor), 0);
    const totalAvulsas = despesasAvulsas.reduce((s, d) => s + num(d.valor), 0);
  ...
  const resumoLimpo = useMemo(() => {
    ...
    return {
      ...r,
      faturamento: fat,
      totalFaturamento: fat.reduce((s, x) => s + num(x.valor), 0),
      totalImpostoFat: fat.reduce((s, x) => s + num(x.imposto), 0),
      folha: fol,
      totalFolha: fol.reduce((s, x) => s + num(x.valor), 0),
      adiantamentos: adi,
      totalAdiantamentos: adi.reduce((s, x) => s + num(x.valor), 0),
      despesasFixas: fix,
      totalFixas: fix.reduce((s, x) => s + num(x.valor), 0),
      despesasAvulsas: av,
      totalAvulsas: av.reduce((s, x) => s + num(x.valor), 0),
      parcelamentos: par,
      totalParcelamentos: par.reduce((s, x) => s + num(x.valor), 0),
    };
  ...
  const importarLancamentos = (rows) => {
    ...
    rows.forEach((dados, idx) => {
      ...
      let horasTrabalhadas = num(dados.horasTrabalhadas);
      if (!horasTrabalhadas) {
        if (ext.inicio && ext.termino) horasTrabalhadas = diffHorasDecimal(ext.inicio, ext.termino);
        else if (ext.inicioMissao && ext.terminoMissao) horasTrabalhadas = diffHorasDecimal(ext.inicioMissao, ext.terminoMissao);
      }
      let kmRodados = num(dados.kmRodados);
      if (!kmRodados && ext.kmInicial != null && ext.kmFinal != null && ext.kmFinal !== '' && ext.kmInicial !== '') kmRodados = Math.max(0, num(ext.kmFinal) - num(ext.kmInicial));
      const lancData = { horasTrabalhadas, kmRodados, pedagio: num(dados.pedagio ?? ext.pedagio), batidaExtra: num(dados.batidaExtra ?? ext.batidaExtra), outros: 0, isDomingo: eDomingo(dados.data) };
      const calc = calcular(s, lancData, t);
      novos.push({ id: `L${baseId}_${idx}`, data: dados.data, codServico: dados.codServico, descricao: s.descricao, cliente: s.cliente, cnpj: s.cnpj, template: s.template, horasTrabalhadas, kmRodados, pedagio: lancData.pedagio, batidaExtra: lancData.batidaExtra, outros: 0, isDomingo: lancData.isDomingo, extras: ext, observacoes: ext.observacao || ext.obs || '', status: 'pendente', ...calc, atualizadoEm: new Date().toISOString() });
    });
    setLancamentos(prev => [...novos, ...prev]);
    showToast(`${novos.length} lançamentos importados${rows.length - novos.length > 0 ? ` (${rows.length - novos.length} ignorados)` : ''}`); setModal(null);
  };
  ...
  const fecharFatura = (fatura) => {
    ...
    const totalImposto = fatura.lancs.reduce((s, l) => s + num(l.imposto), 0);
    const lucroLiq = num(fatura.totalFatura) - num(fatura.totalPago) - totalImposto;
    const f = { id: `F${Date.now()}`, numero, numeroFmt: fmtNumeroFatura(numero), cliente: fatura.cliente, template: fatura.template, periodo: fatura.periodo, dataInicio: fatura.dataInicio || null, dataFim: fatura.dataFim || null, dataFechamento: new Date().toISOString(), totalFatura: fatura.totalFatura, totalPago: fatura.totalPago, totalImposto, lucro: lucroLiq, qtdLancamentos: fatura.qtd, lancamentos: fatura.lancs.map(l => l.id), statusFatura: 'Enviada', historicoStatus: [{ status: 'Enviada', em: new Date().toISOString() }] };
    setFechamentos(prev => [f, ...prev.filter(x => !(x.cliente === f.cliente && x.template === f.template && x.periodo === f.periodo && !x.dataInicio))]);
  ...
  const gerarFaturaCustom = ({ cliente, template, dataInicio, dataFim, competencia }) => {
    const lancsRange = lancamentos.filter(l => l.cliente === cliente && l.template === template && l.data >= dataInicio && l.data <= dataFim);
    if (!lancsRange.length) { showToast('Nenhum lançamento no intervalo', 'error'); return; }
    const totalFatura = lancsRange.reduce((s, l) => s + num(l.totalFatura), 0);
    const totalPago = lancsRange.reduce((s, l) => s + num(l.totalPago), 0);
    const totalImposto = lancsRange.reduce((s, l) => s + num(l.imposto), 0);
    const numero = proximoNumeroFatura();
    const f = { id: `F${Date.now()}`, numero, numeroFmt: fmtNumeroFatura(numero), cliente, template, periodo: competencia, dataInicio, dataFim, dataFechamento: new Date().toISOString(), totalFatura, totalPago, totalImposto, lucro: totalFatura - totalPago - totalImposto, qtdLancamentos: lancsRange.length, lancamentos: lancsRange.map(l => l.id), custom: true, statusFatura: 'Enviada', historicoStatus: [{ status: 'Enviada', em: new Date().toISOString() }] };
  ...
  const exportar = () => {
    const dados = { servicos, lancamentos, fechamentos, funcionarios, folhas, despesas, descontos, exportadoEm: new Date().toISOString(), versao: 4 };
    const blob = new Blob([JSON.stringify(dados, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `fechamento-${hoje()}.json`; a.click(); URL.revokeObjectURL(url);
  ...
  const importar = (e) => {
    ...
    r.onload = (ev) => {
      try {
        ...
        if (d.fechamentos) setFechamentos(d.fechamentos);
        if (d.funcionarios) setFuncionarios(d.funcionarios);
        if (d.folhas) setFolhas(d.folhas);
        if (d.despesas) setDespesas(d.despesas);
        if (d.descontos) setDescontos(d.descontos);
  ...
  const limparTudo = () => {
    setServicos(SERVICOS_INICIAIS); setLancamentos([]); setFechamentos([]);
    setFuncionarios(FUNCIONARIOS_INICIAIS); setFolhas([]); setDespesas([]); setDescontos([]);
    showToast('Resetado'); setModal(null);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 bg-slate-900/50 backdrop-blur sticky top-0 z-40 print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center"><DollarSign className="w-5 h-5" /></div>
            <div>
              <h1 className="font-bold text-lg leading-tight">Fechamento Financeiro</h1>
              <p className="text-xs text-slate-400">{lancamentos.length} lanç · {servicos.length} serv · {funcionarios.length} func · {despesas.length} desp · {descontos.length} desc</p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-xs">
            {erroSalvar ? (
              <button onClick={() => { setErroSalvar(null); Object.entries(pendentesRef.current).forEach(([k, v]) => salvarChave(k, v)); }} className="flex items-center gap-1.5 text-red-400 hover:text-red-300 bg-red-500/10 border border-red-500/30 rounded px-2 py-1" title={erroSalvar.mensagem}><AlertCircle className="w-3.5 h-3.5" />{erroSalvar.tipo === 'quota' ? `"${erroSalvar.key}" muito grande` : 'Erro ao salvar'} — clicar p/ tentar novamente</button>
            ) : salvando ? (
              <span className="flex items-center gap-1.5 text-amber-400"><Save className="w-3.5 h-3.5 animate-pulse" />Salvando...</span>
            ) : (
              <span className="flex items-center gap-1.5 text-emerald-400"><CheckCircle2 className="w-3.5 h-3.5" />Salvo</span>
            )}
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-4 flex gap-1 overflow-x-auto">
          {[
            ...
            { id: 'resumo', l: 'Resumo Fechamento', i: ClipboardList },
            { id: 'lancamentos', l: 'Lançamentos', i: FileText },
            { id: 'despesas', l: 'Despesas', i: TrendingDown },
            { id: 'descontos', l: 'Vales/Adiant.', i: MinusCircle },
            { id: 'faturas', l: 'Faturas', i: Receipt },
            { id: 'funcionarios', l: 'Funcionários', i: Users },
            { id: 'folha', l: 'Folha de Pagto', i: Wallet },
            { id: 'catalogo', l: 'Catálogo', i: Package },
            { id: 'fechamentos', l: 'Fechamentos', i: Archive },
            { id: 'backup', l: 'Backup', i: Database },
          ].map(t => { const I = t.i; return (
            <button key={t.id} onClick={() => setAba(t.id)} className={`px-4 py-2.5 text-sm font-medium border-b-2 transition flex items-center gap-2 whitespace-nowrap ${aba === t.id ? 'border-indigo-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 print:p-0 print:max-w-none">
        ...
        {aba === 'lancamentos' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="relative flex-1 min-w-[200px]"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar..." className="w-full bg-slate-900 border border-slate-800 rounded-lg pl-9 pr-3 py-2 text-sm" /></div>
            ...
            {lancFiltrados.length === 0 ? <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-12 text-center"><p className="text-slate-400">Nenhum lançamento encontrado.</p></div> : (
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="text-xs text-slate-400 bg-slate-900 border-b border-slate-800"><tr><th className="text-left py-3 px-3">Data</th><th className="text-left px-3">Serviço</th><th className="text-left px-3">Cliente</th><th className="text-right px-3">Faturado</th><th className="text-right px-3">Pago</th><th className="text-right px-3">Lucro</th><th className="text-center px-3">Status</th><th className="text-right px-3">Ações</th></tr></thead>
                    <tbody>{lancFiltrados.map(l => (
                      <tr key={l.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="py-2.5 px-3 whitespace-nowrap">{fmtData(l.data)}</td>
                        <td className="px-3"><div className="font-medium">{l.descricao}</div><div className="text-xs text-slate-500">#{l.codServico} · {TEMPLATES[l.template]?.nome}</div></td>
                        <td className="px-3 text-slate-300">{l.cliente}</td><td className="text-right px-3">{fmt(l.totalFatura)}</td>
                        <td className="text-right px-3 text-orange-400">{fmt(l.totalPago)}</td><td className="text-right px-3 text-emerald-400 font-medium">{fmt(l.lucro)}</td>
                    ...
                    <tfoot className="bg-slate-900 border-t border-slate-800 font-medium"><tr><td colSpan={3} className="py-2.5 px-3 text-slate-400">Total ({lancFiltrados.length})</td><td className="text-right px-3">{fmt(totais.fat)}</td><td className="text-right px-3 text-orange-400">{fmt(totais.pag)}</td><td className="text-right px-3 text-emerald-400">{fmt(totais.luc)}</td><td colSpan={2}></td></tr></tfoot>
                  </table>
                </div>
              </div>
            )}
              ...
              <div className="bg-red-500/10 border border-red-500/40 rounded-lg p-4 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-400 mb-3">Para resolver: faça backup completo (aba Backup) e remova despesas antigas que não precisa mais consultar. O storage do navegador tem limite de ~5MB por chave.</p>
                    <div className="flex gap-2 flex-wrap">
                      <button onClick={() => setAba('backup')} className="text-xs bg-slate-700 hover:bg-slate-600 px-3 py-1.5 rounded flex items-center gap-1.5"><Database className="w-3 h-3" />Ir para Backup</button>
                      <button onClick={() => { const corte = new Date(); corte.setMonth(corte.getMonth() - 12); const cmin = corte.toISOString().slice(0, 7); setModal({ tipo: 'confirm', dados: { titulo: `Excluir despesas anteriores a ${fmtMes(cmin)}?`, mensagem: `Serão removidas ${despesas.filter(d => d.competencia < cmin).length} despesas com competência anterior a ${fmtMes(cmin)}. Esta ação não pode ser desfeita — faça backup antes.`, onConfirm: () => { setDespesas(prev => prev.filter(d => d.competencia >= cmin)); setErroSalvar(null); showToast('Despesas antigas removidas'); } } }); }} className="text-xs bg-amber-700 hover:bg-amber-600 px-3 py-1.5 rounded flex items-center gap-1.5"><Trash2 className="w-3 h-3" />Excluir despesas com mais de 12 meses</button>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>{despesasFiltradas.map(d => (
                      <tr key={d.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        ...
                        <td className="px-3 text-xs text-slate-400">{d.centroCusto || '—'}</td>
                        <td className="px-3 text-xs text-slate-400">{d.origem || '—'}</td>
                    ...
                    <tfoot className="bg-slate-900 border-t border-slate-800 font-medium"><tr><td colSpan={5} className="py-2.5 px-3 text-slate-400">Total ({despesasFiltradas.length})</td><td className="text-right px-3 text-red-400">{fmt(totaisDespesas)}</td><td colSpan={2}></td></tr></tfoot>
                  </table>
                </div>
              </div>
            )}
        ...
        {aba === 'descontos' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="relative flex-1 min-w-[200px]"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar beneficiário..." className="w-full bg-slate-900 border border-slate-800 rounded-lg pl-9 pr-3 py-2 text-sm" /></div>
              <select value={filtroCompetencia} onChange={e => setFiltroCompetencia(e.target.value)} className="bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-sm"><option value="">Todas competências</option>{competenciasUsadas.map(c => <option key={c} value={c}>{fmtMes(c)}</option>)}</select>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>{descontosFiltrados.map(d => (
                      <tr key={d.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        ...
                        <td className="px-3 text-xs text-slate-400">{d.centroCusto || '—'}</td>
                        <td className="px-3 text-xs text-slate-400">{d.formaPagamento || '—'}</td>
                    ...
                    <tfoot className="bg-slate-900 border-t border-slate-800 font-medium"><tr><td colSpan={5} className="py-2.5 px-3 text-slate-400">Total ({descontosFiltrados.length})</td><td className="text-right px-3 text-amber-400">{fmt(totaisDescontos)}</td><td></td></tr></tfoot>
                  </table>
                </div>
              </div>
            )}
            ...
            {faturas.length === 0 ? <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-12 text-center"><Receipt className="w-12 h-12 text-slate-600 mx-auto mb-3" /><p className="text-slate-400">Adicione lançamentos para gerar faturas.</p></div> : (
              <div className="grid md:grid-cols-2 gap-3">
                {faturas.filter(f => !filtroCliente || f.cliente === filtroCliente).map(f => {
                  ...
                  const fech = fechamentos.find(x => x.cliente === f.cliente && x.template === f.template && x.periodo === f.periodo);
                  return (
                    <div key={`${f.cliente}|${f.template}|${f.periodo}`} className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          ...
                          <div className="text-xs text-slate-400">{t?.nome} · {fmtPeriodo(f.periodo, t)}</div>
                        </div>
                        {fech ? <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center gap-1"><Lock className="w-3 h-3" />Fechada</span> : <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400">Aberta</span>}
                      </div>
        ...
        {aba === 'funcionarios' && (
          <div className="space-y-4">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="text-xs text-slate-400 bg-slate-900 border-b border-slate-800"><tr><th className="text-left py-3 px-3">Nome</th><th className="text-left px-3">Categoria</th><th className="text-left px-3">CPF</th><th className="text-left px-3">Pix</th><th className="text-center px-3">Status</th><th className="text-right px-3">Ações</th></tr></thead>
                  <tbody>{funcionarios.filter(f => { if (filtroCategoria && f.categoria !== filtroCategoria) return false; if (busca && !`${f.nome} ${f.cpf} ${f.rg}`.toLowerCase().includes(busca.toLowerCase())) return false; return true; }).sort((a, b) => a.nome.localeCompare(b.nome)).map(f => (
                    <tr key={f.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                      ...
                      <td className="px-3 text-slate-400 font-mono text-xs">{f.cpf || '—'}</td>
                      <td className="px-3 text-xs">{f.chavePix ? <span className="text-emerald-400">{f.tipoPix}</span> : <span className="text-slate-500">—</span>}</td>
                      <td className="text-center px-3"><span className={`text-xs px-2 py-0.5 rounded-full ${f.status === 'ATIVO' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400'}`}>{f.status}</span></td>
                      <td className="text-right px-3"><div className="flex justify-end gap-1">
        ...
        {aba === 'folha' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="relative flex-1 min-w-[200px]"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar funcionário..." className="w-full bg-slate-900 border border-slate-800 rounded-lg pl-9 pr-3 py-2 text-sm" /></div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>{folhasFiltradas.map(f => (
                        <tr key={`${f.funcionario.id}|${f.periodo}`} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                          <td className="py-2.5 px-3 font-medium">{f.funcionario.nome}</td>
                          <td className="px-3 text-xs text-slate-400">{f.funcionario.categoria}</td>
                          <td className="px-3 whitespace-nowrap text-xs">{fmtMesCurto(f.periodo)}</td>
                          <td className="text-center px-3">{f.lancs.length}</td>
        ...
        {aba === 'catalogo' && (
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2 items-center justify-between">
              <div><h2 className="text-xl font-bold">Catálogo de Serviços</h2><p className="text-xs text-slate-400">Códigos editáveis. Renomeação atualiza lançamentos automaticamente.</p></div>
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="text-xs text-slate-400 bg-slate-900 border-b border-slate-800"><tr><th className="text-left py-3 px-3">Cód.</th><th className="text-left px-3">Descrição</th><th className="text-center px-3">Categoria</th><th className="text-left px-3">Cliente / Template</th><th className="text-center px-3">Franquia</th><th className="text-right px-3">Diária Pg</th><th className="text-right px-3">Vlr Fatura</th><th className="text-center px-3">Alíquota</th><th className="text-right px-3">Ações</th></tr></thead>
                  <tbody>{servicos.filter(s => {
                  ...
                  }).map(s => { const cat = s.categoriaServico || 'VELADA'; const corC = CORES_CATEGORIA_SERVICO[cat] || CORES_CATEGORIA_SERVICO.VELADA; return (
                    <tr key={s.cod} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                      <td className="py-2.5 px-3 font-mono text-xs text-slate-400">#{s.cod}</td>
                      <td className="px-3 font-medium">{s.descricao}</td>
                      ...
                      <td className="px-3"><div className="text-slate-300">{s.cliente}</div><div className="text-xs text-slate-500">{TEMPLATES[s.template]?.nome}</div></td>
                      <td className="text-center px-3 text-xs text-slate-400">{s.franquiaHoras}h / {s.franquiaKm}km</td>
                      <td className="text-right px-3 text-orange-400">{fmt(s.diariaPaga)}</td>
                      <td className="text-right px-3">{fmt(s.valorFatura)}</td>
                      <td className="text-center px-3"><span className={`text-xs font-mono px-2 py-0.5 rounded ${num(s.aliquota) > 0 ? 'bg-amber-500/20 text-amber-300' : 'bg-slate-700 text-slate-500'}`}>{num(s.aliquota).toFixed(2)}%</span></td>
                      <td className="text-right px-3"><div className="flex justify-end gap-1">
        ...
        {aba === 'resumo' && (
          <div className="space-y-4">
            ...
            {resumoLimpo && (
              <>
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                  <Card title="Faturamento" value={fmt(resumoLimpo.totalFaturamento)} sub={`${resumoLimpo.faturamento.length} operações`} icon={TrendingUp} cor="from-emerald-500/20 to-emerald-500/5" iconCor="text-emerald-400" />
                  <Card title="Folha" value={fmt(resumoLimpo.totalFolha)} sub={`${resumoLimpo.folha.length} funcionários`} icon={Wallet} cor="from-amber-500/20 to-amber-500/5" iconCor="text-amber-400" />
                ...
                <Painel titulo={`1. Faturamento por Cliente — ${fmtMes(competenciaResumo)}`}>
                  ...
                  {resumoFechamento.faturamento.length === 0 ? null : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="text-xs text-slate-400 border-b border-slate-800"><tr><th className="w-8"></th><th className="text-left py-2 px-2">Lançamento</th><th className="text-right px-2">Valor</th><th className="text-center px-2">Cód. Emissão</th><th className="text-center px-2">Qtd</th></tr></thead>
                        <tbody>{resumoFechamento.faturamento.map((f, i) => { const excl = isExclResumo('faturamento', f.key); return (
                          <tr key={f.key} className={`border-b border-slate-800/50 ${excl ? 'opacity-40 line-through' : ''}`}>
                            ...
                            <td className="py-1.5 px-2 font-medium">{f.lancamento}</td>
                            <td className="text-right px-2 text-emerald-400 font-medium">{fmt(f.valor)}</td>
                            <td className="text-center px-2 font-mono text-xs">{f.emissao}</td>
                            <td className="text-center px-2 text-xs text-slate-400">{f.qtd}</td>
                          </tr>
                        ); })}</tbody>
                        <tfoot className="border-t border-slate-700 font-semibold"><tr><td></td><td className="py-2 px-2">TOTAL{(excluidosResumo.faturamento || []).length > 0 ? ' (limpo)' : ''}</td><td className="text-right px-2 text-emerald-400">{fmt(resumoLimpo.totalFaturamento)}</td><td colSpan={2}></td></tr></tfoot>
                      </table>
                    </div>
                  )}
                </Painel>
                ...
                <Painel titulo={`2. Folha de Pagamento — ${fmtMes(competenciaResumo)}`}>
                  <div className="text-xs text-slate-500 italic mb-2">A folha considera apenas a participação em lançamentos que estão dentro de faturas geradas.</div>
                  {resumoFechamento.folha.length === 0 ? <p className="text-sm text-slate-500 italic">Nenhuma folha na competência (gere as faturas para que a folha apareça).</p> : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="text-xs text-slate-400 border-b border-slate-800"><tr><th className="w-8"></th><th className="text-left py-2 px-2">Funcionário</th><th className="text-left px-2">Categoria</th><th className="text-right px-2">Valor</th><th className="text-left px-2">Origem</th></tr></thead>
                        <tbody>{resumoFechamento.folha.map((f, i) => { const excl = isExclResumo('folha', f.key); return (
                          <tr key={f.key} className={`border-b border-slate-800/50 ${excl ? 'opacity-40 line-through' : ''}`}>
                            ...
                            <td className="py-1.5 px-2 font-medium">{f.funcionario}</td>
                            <td className="px-2 text-xs text-slate-400">{f.categoria}</td>
                            <td className="text-right px-2 text-amber-400 font-medium">{fmt(f.valor)}</td>
                            <td className="px-2 text-xs text-slate-400">{f.origem}</td>
                          </tr>
                        ); })}</tbody>
                        <tfoot className="border-t border-slate-700 font-semibold"><tr><td></td><td colSpan={2} className="py-2 px-2">TOTAL{(excluidosResumo.folha || []).length > 0 ? ' (limpo)' : ''}</td><td className="text-right px-2 text-amber-400">{fmt(resumoLimpo.totalFolha)}</td><td></td></tr></tfoot>
                      </table>
                    </div>
                  )}
                </Painel>
        ...
        {aba === 'backup' && (
          <div className="space-y-4 max-w-2xl">
            ...
            <Painel titulo="Resumo dos dados">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs">
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{servicos.length}</div><div className="text-slate-400">Serviços</div></div>
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{lancamentos.length}</div><div className="text-slate-400">Lançamentos</div></div>
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{funcionarios.length}</div><div className="text-slate-400">Funcionários</div></div>
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{folhas.length}</div><div className="text-slate-400">Folhas</div></div>
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{despesas.length}</div><div className="text-slate-400">Despesas</div></div>
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{descontos.length}</div><div className="text-slate-400">Descontos</div></div>
                <div className="bg-slate-800/50 rounded-lg p-2"><div className="text-xl font-bold">{fechamentos.length}</div><div className="text-slate-400">Fechamentos</div></div>
              </div>
            </Painel>
            <Painel titulo="Diagnóstico de armazenamento">
              <p className="text-xs text-slate-400 mb-3">Cada chave tem limite de ~5 MB no storage do navegador. Se algo passar disso, o save falha.</p>
              <table className="w-full text-xs">
                <thead><tr className="text-slate-500 text-[10px] uppercase border-b border-slate-700"><th className="text-left py-1.5">Chave</th><th className="text-right">Registros</th><th className="text-right">Tamanho</th><th className="text-right">Uso</th></tr></thead>
                <tbody>{[
                  ['servicos', servicos], ['lancamentos', lancamentos], ['fechamentos', fechamentos],
                  ['funcionarios', funcionarios], ['folhas', folhas], ['despesas', despesas], ['descontos', descontos],
                ].map(([key, arr]) => {
                  const tam = JSON.stringify(arr).length;
                  const pct = (tam / LIMITE_STORAGE_BYTES) * 100;
                  const cor = pct > 80 ? 'text-red-400' : pct > 50 ? 'text-amber-400' : 'text-emerald-400';
                  const barColor = pct > 80 ? 'bg-red-500' : pct > 50 ? 'bg-amber-500' : 'bg-emerald-500';
                  return (
                    <tr key={key} className="border-b border-slate-800/50">
                      <td className="py-1.5 font-medium">{key}</td>
                      <td className="text-right text-slate-400">{Array.isArray(arr) ? arr.length : '—'}</td>
                      <td className={`text-right font-mono ${cor}`}>{tam < 1024 ? `${tam} B` : tam < 1024 * 1024 ? `${(tam / 1024).toFixed(1)} KB` : `${(tam / 1024 / 1024).toFixed(2)} MB`}</td>
                      <td className="text-right pl-3"><div className="flex items-center justify-end gap-2"><span className={`text-[10px] ${cor}`}>{pct.toFixed(1)}%</span><div className="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full ${barColor}`} style={{ width: `${Math.min(100, pct)}%` }} /></div></div></td>
                    </tr>
                  );
                })}</tbody>
              </table>
            </Painel>
            <Painel titulo="Exportar"><p className="text-sm text-slate-400 mb-3">Gera JSON com todo o banco. Suba no Drive como backup.</p><button onClick={exportar} className="bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2"><Download className="w-4 h-4" />Exportar backup</button></Painel>
            <Painel titulo="Importar"><p className="text-sm text-slate-400 mb-3">Restaura tudo de um arquivo exportado.</p><label className="bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-lg text-sm font-medium inline-flex items-center gap-2 cursor-pointer"><Upload className="w-4 h-4" />Selecionar arquivo<input type="file" accept=".json" onChange={importar} className="hidden" /></label></Painel>
            <Painel titulo="Zona de risco"><p className="text-sm text-slate-400 mb-3">Apaga todos os dados.</p><button onClick={() => setModal({ tipo: 'confirmLimpar' })} className="bg-red-600 hover:bg-red-500 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2"><Trash2 className="w-4 h-4" />Limpar todos os dados</button></Painel>
          </div>
        )}
      </main>

      {toast && <div className={`fixed bottom-6 right-6 px-4 py-3 rounded-lg shadow-2xl z-50 flex items-center gap-2 text-sm font-medium print:hidden ${toast.tipo === 'error' ? 'bg-red-600' : 'bg-emerald-600'}`}>{toast.tipo === 'error' ? <AlertCircle className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}{toast.msg}</div>}
      ...
      {modal?.tipo === 'lancamento' && <ModalLancamento dados={modal.dados} servicos={servicos} funcionarios={funcionarios} onSave={salvarLancamento} onClose={() => setModal(null)} />}
      ...
      {modal?.tipo === 'confirmFechar' && <ModalConfirm titulo="Fechar fatura?" mensagem={`${modal.dados.cliente} · ${fmtPeriodo(modal.dados.periodo, TEMPLATES[modal.dados.template])}.`} onConfirm={() => fecharFatura(modal.dados)} onClose={() => setModal(null)} cor="emerald" />}
      ...
      {modal?.tipo === 'confirmLimpar' && <ModalConfirm titulo="Limpar todos os dados?" mensagem="Esta ação não pode ser desfeita." onConfirm={limparTudo} onClose={() => setModal(null)} />}
    </div>
  );
}
...
// ============ MODAL LANÇAMENTO ============
function ModalLancamento({ dados, servicos, funcionarios, onSave, onClose }) {
  const [form, setForm] = useState(dados ? { ...dados, extras: dados.extras || {} } : { data: hoje(), codServico: '', horasTrabalhadas: 0, kmRodados: 0, pedagio: 0, batidaExtra: 0, outros: 0, isDomingo: false, extras: {}, observacoes: '', status: 'pendente' });
  ...
  useEffect(() => {
    ...
    if (e.inicio && e.termino) { const h = diffHorasDecimal(e.inicio, e.termino); if (h !== num(form.horasTrabalhadas)) updates.horasTrabalhadas = h.toFixed(2); }
    if (e.inicioMissao && e.terminoMissao) { const h = diffHorasDecimal(e.inicioMissao, e.terminoMissao); if (h !== num(form.horasTrabalhadas)) updates.horasTrabalhadas = h.toFixed(2); }
    if (e.kmInicial != null && e.kmFinal != null && e.kmFinal !== '' && e.kmInicial !== '') { const km = Math.max(0, num(e.kmFinal) - num(e.kmInicial)); if (km !== num(form.kmRodados)) updates.kmRodados = km; }
    if (Object.keys(updates).length) setForm(f => ({ ...f, ...updates }));
  }, [form.extras]);

  useEffect(() => { if (form.data) setForm(f => ({ ...f, isDomingo: eDomingo(form.data) })); }, [form.data]);
      ...
      {servico && template && (
        <>
          ...
          <div className="mt-3 grid sm:grid-cols-3 gap-3">
            <Campo label="Horas trabalhadas"><input type="number" step="0.01" value={form.horasTrabalhadas} onChange={e => setForm({ ...form, horasTrabalhadas: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm" /></Campo>
            ...
            <div className="sm:col-span-2 flex items-center pt-6"><label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={!!form.isDomingo} onChange={e => setForm({ ...form, isDomingo: e.target.checked })} className="w-4 h-4 rounded" />Aplicar adicional de domingo {eDomingo(form.data) && <span className="text-xs text-amber-400">(detectado)</span>}</label></div>
          </div>
...
function ModalServico({ dados, clientes = [], onSave, onClose }) {
  const [f, setF] = useState(dados || { cod: '', template: 'TOMBINI', descricao: '', cliente: '', cnpj: '', emissao: '', franquiaHoras: 0, franquiaKm: 0, horaExtraFatura: 0, kmExtraFatura: 0, diariaPaga: 0, horaExtraPaga: 0, kmExtraPago: 0, adicionalDomingosFatura: 0, adicionalDomingosPago: 0, valorFatura: 0, aliquota: 0, categoriaServico: 'VELADA', status: 'ATIVO' });
  const [clienteNovo, setClienteNovo] = useState(false);
  const isEdit = !!dados;
  const codAnterior = dados?.cod;
  // Auto-preenche cliente quando troca template (apenas se ainda não foi setado).
  useEffect(() => { const t = TEMPLATES[f.template]; if (t && !isEdit && !f.cliente) setF(s => ({ ...s, cliente: t.cliente })); }, [f.template]);
  // Detecta cliente personalizado (não está na lista) para abrir já em modo "novo"
  ...
  return (
    <ModalBase titulo={isEdit ? 'Editar serviço' : 'Novo serviço'} onClose={onClose}>
      <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-lg p-3 mb-4">
        ...
        {isEdit && codAnterior !== f.cod && <p className="text-[11px] text-amber-400 mt-1.5">⚠ Você está renomeando de <b>#{codAnterior}</b> para <b>#{f.cod}</b>. Todos os lançamentos serão atualizados.</p>}
      </div>
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <Campo label="Categoria do serviço"><select value={f.categoriaServico || 'VELADA'} onChange={e => setF({ ...f, categoriaServico: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm">{CATEGORIAS_SERVICO.map(c => <option key={c} value={c}>{c}</option>)}</select></Campo>
        ...
        <Campo label="Descrição" full><input value={f.descricao} onChange={e => setF({ ...f, descricao: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm" /></Campo>
        <Campo label="Cliente">
          {clienteNovo ? (
            <div className="flex gap-1">
              <input value={f.cliente} onChange={e => setF({ ...f, cliente: e.target.value })} placeholder="Digite o nome do novo cliente" className="flex-1 bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm" autoFocus />
              <button type="button" onClick={() => { setClienteNovo(false); setF({ ...f, cliente: '' }); }} title="Voltar para a lista" className="px-2 bg-slate-700 hover:bg-slate-600 rounded text-xs">↩</button>
            </div>
          ) : (
        ...
        {N('valorFatura', 'Vlr Fatura (base)')}{N('diariaPaga', 'Diária paga')}
        <div className="sm:col-span-2 bg-amber-500/10 border border-amber-500/30 rounded p-3 mt-2">
          <label className="block text-xs uppercase font-semibold text-amber-300 mb-1.5">Alíquota de imposto (%)</label>
          <div className="flex items-center gap-2">
            <input type="number" step="0.01" value={f.aliquota} onChange={e => setF({ ...f, aliquota: e.target.value })} placeholder="Ex: 15.60" className="w-32 bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm font-mono" />
            <span className="text-xs text-slate-400">% sobre o faturado · só impacta cálculo de lucro · não aparece como despesa</span>
          </div>
          <p className="text-[10px] text-slate-500 mt-1.5">⚠ Editar a alíquota não altera faturas e folhas já fechadas — afeta apenas lançamentos novos a partir daqui.</p>
        </div>
        <div className="sm:col-span-2 text-xs uppercase text-slate-500 font-semibold mt-2 pt-2 border-t border-slate-700">Franquia</div>
        {N('franquiaHoras', 'Franquia horas', '1')}{N('franquiaKm', 'Franquia km', '1')}
        <div className="sm:col-span-2 text-xs uppercase text-slate-500 font-semibold mt-2 pt-2 border-t border-slate-700">Faturamento (extras)</div>
        {N('horaExtraFatura', 'Hora extra fatura')}{N('kmExtraFatura', 'Km extra fatura')}{N('adicionalDomingosFatura', 'Adic. domingo fatura')}
        <div className="sm:col-span-2 text-xs uppercase text-slate-500 font-semibold mt-2 pt-2 border-t border-slate-700">Pagamento (extras)</div>
        {N('horaExtraPaga', 'Hora extra paga')}{N('kmExtraPago', 'Km extra pago')}{N('adicionalDomingosPago', 'Adic. domingo pago')}
      </div>
      <div className="flex justify-end gap-2 mt-4"><button onClick={onClose} className="px-4 py-2 rounded-lg text-sm bg-slate-700 hover:bg-slate-600">Cancelar</button><button onClick={submit} className="px-4 py-2 rounded-lg text-sm bg-indigo-600 hover:bg-indigo-500 font-medium">Salvar</button></div>
    </ModalBase>
  );
...
function ModalFaturaIntervalo({ clientes, servicos, proximoNumero, onSave, onClose }) {
  const hojeStr = hoje();
  ...
  const templatesPorCliente = useMemo(() => {
    if (!cliente) return [];
    return [...new Set(servicos.filter(s => s.cliente === cliente).map(s => s.template))];
  }, [cliente, servicos]);
  const submit = () => {
    if (!cliente) return alert('Selecione o cliente');
    if (!template) return alert('Selecione o template');
    ...
    onSave({ cliente, template, dataInicio, dataFim, competencia });
  };
  return (
    <ModalBase titulo="Gerar fatura por intervalo de datas" onClose={onClose}>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <Campo label="Cliente *">
          <select value={cliente} onChange={e => { setCliente(e.target.value); setTemplate(''); }} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm">
            <option value="">Selecione...</option>
            {clientes.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </Campo>
        <Campo label="Template *">
          <select value={template} onChange={e => setTemplate(e.target.value)} disabled={!cliente} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm disabled:opacity-50">
            <option value="">{cliente ? 'Selecione...' : 'Escolha cliente primeiro'}</option>
            {templatesPorCliente.map(tid => <option key={tid} value={tid}>{TEMPLATES[tid]?.nome || tid}</option>)}
          </select>
        </Campo>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style={{ textAlign: 'right', width: '70px' }}>Total Pago</th>
                <th style={{ textAlign: 'center', width: '36px' }}>Eq.</th>
                <th style={{ textAlign: 'right', width: '80px' }}>Sua parte</th>
              </tr>
            </thead>
            <tbody>
              {c.items.sort((a, b) => a.data.localeCompare(b.data)).map(l => (
                <tr key={l.id}>
                  ...
                  <td style={{ textAlign: 'right' }}>{fmt(l.totalPago)}</td>
                  <td style={{ textAlign: 'center' }}>{l.qtdAg}</td>
                  <td style={{ textAlign: 'right', fontWeight: 'bold' }}>{fmt(l.participacao)}</td>
                </tr>
              ))}
              <tr style={{ background: '#f8fafc', fontWeight: 'bold' }}>
                <td colSpan={7} style={{ textAlign: 'right' }}>Subtotal {c.cliente}:</td>
...
function ModalBase({ titulo, children, onClose, pequeno, grande }) {
  const w = pequeno ? 'max-w-md' : grande ? 'max-w-4xl' : 'max-w-2xl';
  return <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}><div onClick={e => e.stopPropagation()} className={`bg-slate-900 border border-slate-700 rounded-xl p-5 w-full ${w} my-8`}><div className="flex items-center justify-between mb-4"><h3 className="font-bold text-lg">{titulo}</h3><button onClick={onClose} className="p-1 hover:bg-slate-800 rounded"><X className="w-5 h-5" /></button></div>{children}</div></div>;
}
...
function parseDateTimeBR(s) { if (!s) return null; const m = s.match(/(\d{1,2})\/(\d{1,2})\/(\d{2,4})\s+(\d{1,2}):(\d{2})/); if (!m) { const date = parseDataBR(s); return date ? `${date}T00:00` : null; } let [, d, mn, y, h, mi] = m; if (y.length === 2) y = '20' + y; return `${y}-${mn.padStart(2, '0')}-${d.padStart(2, '0')}T${h.padStart(2, '0')}:${mi}`; }
...
function buildHeaderMapLanc(template) {
  ...
  ['DIA', 'TOTALDEHORAS', 'HORASTRABALHADAS', 'HORAEXTRA', 'HE', 'KMTOTAL', 'KMEXTRA', 'FRANQUIAHR', 'FRANQUIAHORA', 'FRANQUIAKM'].forEach(k => map[k] = '@skip');
  return map;
}
...
function parseRowLanc(row, headers, headerMap, template) {
  ...
  headers.forEach((h, i) => {
    ...
    else if (field === '@codServico') dados.codServico = value;
    else if (field.startsWith('extras.')) {
      const key = field.substring(7); const campo = template.campos.find(c => c.k === key);
      if (campo?.tipo === 'datetime') { const dt = parseDateTimeBR(value); if (dt) dados.extras[key] = dt; }
      else if (campo?.tipo === 'time') { if (value.match(/^\d{1,2}:\d{2}$/)) dados.extras[key] = value.length === 4 ? '0' + value : value; }
