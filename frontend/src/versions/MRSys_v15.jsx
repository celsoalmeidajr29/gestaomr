// v15 — exibe número de versão no cabeçalho
...
export default function App() {
  ...
  const folhasPorFunc = useMemo(() => {
    ...
    return Object.values(m).map(g => {
      ...
      const totalDiariasAvulsas = diariasPeriodo.reduce((s, d) => s + num(d.valor), 0);
      ...
      const bruto = g.total + salarioFixoMes + totalDiariasAvulsas + adicionais;
      return { ...g, ajustes, vales: valesPeriodo, totalVales, diariasPeriodo, totalDiariasAvulsas, adicionais, descontos: descontosTotal, descontosManuais, salarioFixo: salarioFixoMes, bruto, liquido: bruto - descontosTotal, status: folha?.status || 'aberta', folhaId: folha?.id };
      ...
      <header className="border-b border-slate-800 bg-slate-900/50 backdrop-blur sticky top-0 z-40 print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 text-xs">
            <span className="text-slate-600 font-mono text-[10px] select-none">{APP_VERSION}</span>
            {erroSalvar ? (
        ...
        <div className="max-w-7xl mx-auto px-4 flex gap-1 overflow-x-auto">
          {[
            ...
            { id: 'diarias', l: 'Diárias Avulsas', i: Calendar },
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="text-xs text-slate-400 bg-slate-900 border-b border-slate-800">
                          <tr><th className="text-left py-3 px-3">Data</th><th className="text-left px-3">Compet.</th><th className="text-left px-3">Funcionário</th><th className="text-left px-3">Cliente</th><th className="text-right px-3">Valor</th><th className="text-right px-3">Ações</th></tr>
                        </thead>
                        <tbody>{diariasFiltradas.map(d => (
                          <tr key={d.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                            <td className="py-2.5 px-3 text-xs whitespace-nowrap">{d.data ? d.data.split('-').reverse().join('/') : '—'}</td>
                            <td className="px-3 text-xs text-slate-300">{fmtMesCurto(d.competencia)}</td>
                            <td className="px-3 font-medium">{d.nome}</td>
                            <td className="px-3 text-xs text-slate-400">{d.clienteNome || '—'}</td>
                        ...
                        <tfoot className="bg-slate-900 border-t border-slate-800 font-medium"><tr><td colSpan={4} className="py-2.5 px-3 text-slate-400">Total ({diariasFiltradas.length})</td><td className="text-right px-3 text-orange-400">{fmt(totalDiarias)}</td><td></td></tr></tfoot>
                      </table>
                    </div>
                  </div>
                )
