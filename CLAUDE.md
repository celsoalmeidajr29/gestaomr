...
- **v1.0.25** — Mensagens pré-definidas para propostas: botão "Mensagens" na aba Propostas abre `ModalMensagensPadrao` (CRUD por categoria: Objeto/Faturamento/Prazos/Observações). Nos 4 campos textarea do `ModalProposta`, botão BookOpen abre picker inline que insere o texto pré-definido (append se o campo já tiver conteúdo). Backend: `migration 026` cria `proposta_mensagens_padrao` + endpoint `GET/POST/PUT/DELETE /api/propostas/mensagens.php`. Fix: e-mail de medição agora envia UM único e-mail para todos os destinatários (campo `To:` com todos os endereços separados por vírgula, em vez de um loop com mail() individual por destinatário).
...
- **Pare Certo - Análises (v0.1, 2026-05-11):** segundo sistema no Hub. Card emerald→teal, permissão `pareceto` (admin bypass). Frontend em `frontend/src/pareceto/PareCetoApp.jsx` (monolito isolado — sem comunicação com MRSys). Backend em `backend/api/pareceto/` (usa mesmo MySQL com tabelas `pc_*`). Migration 014 cria as tabelas base (`pc_funcionarios`, `pc_relatorios_historico`). Migration 019 adiciona `hash_dedup`/`nome_arquivo`/`importado_em` em `pc_vendas` (dedup por hash em vez de placa — mesma placa pode ter N transações). Migration 020 cria `pc_metas` (metas mensais por funcionário, UNIQUE por nome+mês). Backend `backend/api/pareceto/metas/` (CRUD + upsert ON DUPLICATE KEY).
  - **Fase 1–3 (v0.1):** Módulo 1 Vendas (CSV TAB, dedup placa, canal/zona/tipo/agente/jornada), Módulo 2 Irregularidades (CSV semicolon, semana ISO, rankings, Top 20 placas), Módulo 3 Funcionários (CRUD, vínculo de login com CSVs), Histórico. Exports XLSX + TXT em todos os módulos.
  - **Fase 4:** Drill-down individual de agente em Vendas — clicar numa linha de `VendasAgentesChart` abre painel com KPIs, gráfico temporal e tabela de transações filtrados para aquele agente. Prop `onSelectAgente` + coluna `›` na tabela.
  - **Fase 5:** Score engine de risco de placa — 4 fatores ponderados (volume, % irregular, reincidência 90d, recência com decay exponencial) → score 0–100 normalizado. Níveis: crítico/alto/médio/baixo. `ScoreBadge` e `MiniScoreBar` usados nas listagens. Alertas automáticos via `AlertasPanel` (badge com qtd críticas no tab Irregularidades, dismissível). Módulo 7 Configurações: sliders para pesos do score, metas mensais por agente/mês (CRUD persistido em `pc_metas` via API), aba de exportar dados. Config do score persiste em `localStorage` (`pc_score_config`).
  - **Fase 6:** Comparação de períodos (`ComparacaoPeriodos`) disponível em Vendas e Irregularidades — Período A vs B com delta badges (▲/▼), KPIs comparados, ranking de trechos e agentes. Inicializa automático com 1ª e 2ª metade do range de dados. Relatório Semanal (`RelatorioSemanal`) com seletor de semana ISO, análise completa e exports XLSX + TXT.
  - **Fase 7:** Módulo de inadimplência (`AbaInadimplencia`) — calcula transações irregulares não quitadas com aging em 4 faixas (recente ≤30d, moderado 31–60d, antigo 61–90d, crítico >90d). Totais, valor pendente, distribuição por trecho, lista de placas com score integrado. Comparação de Trechos (`ComparacaoTrechos`) — seleciona até 8 trechos, escolhe métrica (transações/receita/irregulares/% irreg.) e plota BarChart horizontal lado a lado.
  - **Fase 8 — Dashboard Executivo (tela inicial):** `AbaDashboard` é a primeira aba carregada. 6 KPI cards (transações, receita, notificações, valor pendente, placas críticas, agentes ativos). Smart insights automáticos em badges coloridos (ok/alerta/crítico/info) gerados a partir do estado dos dados. 3 gráficos: receita mensal (BarChart), notificações semanais (AreaChart), distribuição de risco (PieChart donut). Rankings Top 5 agentes e Top 5 trechos. Cross-analysis receita vs pendente por trecho (aparece apenas quando Vendas + Irregularidades carregados). Aging bars de inadimplência. Estado vazio orienta importação quando nenhum dado está carregado.
...
*Última atualização: 2026-05-16. MRSys v1.0.25 em produção. Cérebro v1.5.0. Pare Certo v0.4.0. Dark mode toggle Sol/Lua em TODOS os sistemas + Hub + Login (tema persiste em `localStorage` chave `mr-theme`, compartilhada). Auditoria completa de tema claro/escuro concluída:*
...
- *Rodar migration 019 (`database/migrations/019_pc_vendas_hash_dedup.sql`) para dedup por hash (permite N transações por placa). Inclui backfill e troca de UNIQUE key.*
- *Rodar migration 020 (`database/migrations/020_pc_metas.sql`) para criar `pc_metas` (metas mensais por agente — usada na aba Configurações > Metas).*
